import { z } from 'zod';
import { FrequencyType, EventType } from '@/app/constants/form.constants';

export const eventFormSchema = z.object({
  type: z.enum([EventType.RENDA, EventType.DESPESA], {message: 'Selecione uma opção válida'}),
  frequency: z.enum([FrequencyType.UNICA, FrequencyType.MENSAL], {message: 'Selecione uma opção válida'}),
  ...
  if (data.frequency === FrequencyType.UNICA) {
  ...
  if (data.frequency === FrequencyType.MENSAL) {
  ...
  if (data.frequency === FrequencyType.MENSAL && data.start_year && data.end_year) {
